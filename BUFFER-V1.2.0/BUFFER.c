...
static struct ControllFlag /* Pointer for buffer control */
{
 
 uint8_t BufferEnd : 1;    /* This flag set when counter buffer is end */
 uint8_t OvfControll : 3;  /* This flag is for control the write overflow */
...
static uint8_t buffer[_BUFFER_LENGTH]; /* Array for save data */
...
void Save_In_Buffer( uint8_t data_for_save ) /* Function for save data in the buffer */
{
	...
	if( (( write_counter >= read_counter ) & ( BufferControll.BufferEnd == 0 )) | (( write_counter < read_counter ) & ( BufferControll.BufferEnd == 1 )) ) /* Buffer scrutiny */
	{
		...
		if( write_counter == ( _BUFFER_LENGTH - 1 ) ) /* Data number scrutiny for next write */
		{
		...
		if( available_data_quantity == _BUFFER_LENGTH ) /* available data scrutiny for set 'buffer is full' bit */
		{
	...
	else
	{
		...
		if( BufferControll.OvfControll == _NUMBER_OF_OVERFLOW ) /* number of overflow scrutiny */
		{
...
uint16_t Available_In_Buffer(void) /* Function for check number of data in the buffer */
{
	...
	if( ( write_counter >= read_counter ) & ( BufferControll.BufferEnd == 0 ) & ( BufferControll.OvfControll < _NUMBER_OF_OVERFLOW ) ) /* Buffer scrutiny */
	{
	...
	else if( ( write_counter <= read_counter ) & ( BufferControll.BufferEnd == 1 ) & ( BufferControll.OvfControll < _NUMBER_OF_OVERFLOW ) ) /* Buffer scrutiny */
	{
		
		return (( _BUFFER_LENGTH - read_counter ) + write_counter ); /* Return number of data */
		
	}
	
	else if( ( write_counter >= read_counter ) & ( BufferControll.BufferEnd == 0 ) & ( BufferControll.OvfControll == _NUMBER_OF_OVERFLOW ) ) /* Buffer scrutiny */
	{
	...
	if( available_data_quantity >= number_of_read ) /* Number of data scrutiny */
	{
		...
		for( ; number_of_read > 0 ; number_of_read-- ) /* Loop for data copy */
		{
			...
			*location_for_save_readed_data = buffer[ read_counter ]; /* Copy data in the data location */
			...
			if( read_counter == _BUFFER_LENGTH ) /* Check for end buffer length */
			{
...
void Increment_Read_Index( uint16_t number_of_increment ) /* Function for index increment */
{
	...
	if( available_data_quantity >= number_of_increment ) /* Number of data scrutiny */
	{
		...
		if( read_counter >= _BUFFER_LENGTH ) /* Check buffer and set the length */
		{
			
			BufferControll.BufferEnd = 0; /* Reset 'buffer end' flag */
			read_counter -= _BUFFER_LENGTH; /* Set the length */
...
void Clean_Buffer(void) /* Function for clean buffer */
{
	...
	write_counter = 0; /* Reset value */
	read_counter = 0; /* Reset value */
	
	/* ----------- Reset values ----------- */
	
	BufferControll.BufferEnd = _FALSE; /* Reset flag */
	BufferControll.OvfControll = _FALSE; /* Reset flag */
	...
	while ( cleaner_counter < _BUFFER_LENGTH ) /* Loop for clean buffer data */
	{
		...
		buffer[cleaner_counter] = NULL; /* Set value */
